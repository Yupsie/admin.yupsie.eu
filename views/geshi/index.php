 ...
 *
 *	The GeSHi class for Generic Syntax Highlighting. Please refer to the documentation at http://qbnz.com/highlighter/documentation.php for more
 *	information about how to use this class. For changes, release notes, TODOs etc, see the relevant files in the docs/ directory.
 *
 *	This file is part of GeSHi.
 *
 *  GeSHi is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by
 *  the Free Software Foundation; either version 2 of the License, or (at your option) any later version.
 *
 *  GeSHi is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or 
 *	FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.
 *
 *  You should have received a copy of the GNU General Public License along with GeSHi; if not, write to the Free Software Foundation, Inc., 
 ...
 */
define('GESHI_VERSION', '1.0.8.9');					//	The version of this GeSHi file

if (!defined('GESHI_ROOT')) {						//	The root directory for GeSHi
    define('GESHI_ROOT', dirname(__FILE__) . DIRECTORY_SEPARATOR);
}
define('GESHI_LANG_ROOT', GESHI_ROOT . 'geshi' . DIRECTORY_SEPARATOR);	//	The language file directory for GeSHi

if (!defined('GESHI_SECURITY_PARANOID')) {
    define('GESHI_SECURITY_PARANOID', false);		//	Define if GeSHi should be paranoid about security
}

define('GESHI_NO_LINE_NUMBERS', 0);					//	Use no line numbers when building the result
define('GESHI_NORMAL_LINE_NUMBERS', 1);				//	Use normal line numbers when building the result
define('GESHI_FANCY_LINE_NUMBERS', 2);				//	Use fancy line numbers when building the result

define('GESHI_HEADER_NONE', 0);						//	Use nothing to surround the source
define('GESHI_HEADER_DIV', 1);						//	Use a "div" to surround the source
define('GESHI_HEADER_PRE', 2);						//	Use a "pre" to surround the source
define('GESHI_HEADER_PRE_VALID', 3);				//	Use a pre to wrap lines when line numbers are enabled or to wrap the whole code.
define('GESHI_HEADER_PRE_TABLE', 4);				//	Use a "table" to surround the source

define('GESHI_CAPS_NO_CHANGE', 0);					//	Lowercase keywords found
define('GESHI_CAPS_UPPER', 1);						//	Uppercase keywords found
define('GESHI_CAPS_LOWER', 2);						//	Leave keywords found as the case that they are

define('GESHI_LINK', 0);							//	Links in the source in the :link state
define('GESHI_HOVER', 1);							//	Links in the source in the :hover state
define('GESHI_ACTIVE', 2);							//	Links in the source in the :active state
define('GESHI_VISITED', 3);							//	Links in the source in the :visited state

//	Note that if you change these, they should be as-is: i.e., don't write them as if they had been run through htmlentities()
define('GESHI_START_IMPORTANT', '<BEGIN GeSHi>');	//	The starter for important parts of the source
define('GESHI_END_IMPORTANT', '<END GeSHi>');		//	The ender for important parts of the source

define('GESHI_NEVER', 0);							//	Strict mode never applies (this is the most common)
define('GESHI_MAYBE', 1);							//	Strict mode *might* apply, and can be enabled or disabled by {@link GeSHi->enable_strict_mode()}
define('GESHI_ALWAYS', 2);							//	Strict mode always applies

define('GESHI_SEARCH', 0);							//	Key of regex array defining what to search for
define('GESHI_REPLACE', 1);							//	Key of regex array defining what bracket group in a matched search to use as a replacement
define('GESHI_MODIFIERS', 2);						//	Key of regex array defining any modifiers to the regular expression
define('GESHI_BEFORE', 3);							//	Key of regex array defining what bracket group in a matched search to put before the replacement
define('GESHI_AFTER', 4);							//	Key of regex array defining what bracket group in a matched search to put after the replacement
define('GESHI_CLASS', 5);							//	Key of regex array defining a custom keyword to use for this regexp's html tag class

define('GESHI_COMMENTS', 0);						//	Used in language files to mark comments

define('GESHI_PHP_PRE_433', !(version_compare(PHP_VERSION, '4.3.3') === 1));	//	Used to work around missing PHP features
...
define('GESHI_MAX_PCRE_SUBPATTERNS', 500);			//	Some old subpatterns only support up to xxx subpatterns in regexes. Set this to false if your lib is up to date
define('GESHI_MAX_PCRE_LENGTH', 12288);				//	Also important not to generate too long regexes. 12k should do just fine on a 16k limit.

define('GESHI_NUMBER_INT_BASIC', 1);				//	Basic. Default integers \d+
define('GESHI_NUMBER_INT_CSTYLE', 2);				//	Enhanced. Default C-Style \d+[lL]?
define('GESHI_NUMBER_BIN_SUFFIX', 16);				//	Number format to highlight binary numbers with a suffix "b". [01]+[bB]
define('GESHI_NUMBER_BIN_PREFIX_PERCENT', 32);		//	Number format to highlight binary numbers with a prefix %. %[01]+
define('GESHI_NUMBER_BIN_PREFIX_0B', 64);			//	Number format to highlight binary numbers with a prefix 0b (C). 0b[01]+
define('GESHI_NUMBER_OCT_PREFIX', 256);				//	Number format to highlight octal numbers with a leading zero. 0[0-7]+
define('GESHI_NUMBER_OCT_PREFIX_0O', 512);			//	Number format to highlight octal numbers with a prefix 0o (logtalk). 0[0-7]+
define('GESHI_NUMBER_OCT_PREFIX_AT', 1024);			//	Number format to highlight octal numbers with a leading @ (Used in HiSofts Devpac series). @[0-7]+
define('GESHI_NUMBER_OCT_SUFFIX', 2048);			//	Number format to highlight octal numbers with a suffix of o. [0-7]+[oO]
define('GESHI_NUMBER_HEX_PREFIX', 4096);			//	Number format to highlight hex numbers with a prefix 0x. 0x[0-9a-fA-F]+
define('GESHI_NUMBER_HEX_PREFIX_DOLLAR', 8192);		//	Number format to highlight hex numbers with a prefix $. $[0-9a-fA-F]+
define('GESHI_NUMBER_HEX_SUFFIX', 16384);			//	Number format to highlight hex numbers with a suffix of h. [0-9][0-9a-fA-F]*h
define('GESHI_NUMBER_FLT_NONSCI', 65536);			//	Number format to highlight floating-point numbers without support for scientific notation. \d+\.\d+
define('GESHI_NUMBER_FLT_NONSCI_F', 131072);		//	Number format to highlight floating-point numbers without support for scientific notation. \d+(\.\d+)?f
define('GESHI_NUMBER_FLT_SCI_SHORT', 262144);		//	Number format to highlight floating-point numbers with support for scientific notation (E) and optional leading zero. \.\d+e\d+
define('GESHI_NUMBER_FLT_SCI_ZERO', 524288);		//	Number format to highlight floating-point numbers with support for scientific notation (E) and required leading digit. \d+(\.\d+)?e\d+

define('GESHI_ERROR_NO_INPUT', 1);					//	No sourcecode to highlight was specified @deprecated
define('GESHI_ERROR_NO_SUCH_LANG', 2);				//	The language specified does not exist
define('GESHI_ERROR_FILE_NOT_READABLE', 3);			//	GeSHi could not open a file for reading (generally a language file)
define('GESHI_ERROR_INVALID_HEADER_TYPE', 4);		//	The header type passed to {@link GeSHi->set_header_type()} was invalid
define('GESHI_ERROR_INVALID_LINE_NUMBER_TYPE', 5);	//	The line number type passed to {@link GeSHi->enable_line_numbers()} was invalid


/**
 *	class Views_Geshi_Main
 *	Documentation for GeSHi 1.0.X is available at http://qbnz.com/highlighter/documentation.php for more information about how to use this class.
 *
 *	@package   geshi
 *	@author    Nigel McNie <nigel@geshi.org>, Benny Baumann <BenBE@omorphia.de>
 *	@copyright (C) 2004 - 2007 Nigel McNie, (C) 2007 - 2008 Benny Baumann
 */
class V_Geshi_Main {
    private $source = '';							//	The source code to highlight
    public $language = '';							//	The language to use when highlighting
    public $language_data = array();				//	The data for the language used
    public $language_path = GESHI_LANG_ROOT;		//	The path to the language files
    public $error = false;							//	The error message associated with an error
    ...
    public $strict_mode = false;					//	Whether highlighting is strict or not
    public $use_classes = false;					//	Whether to use CSS classes in output
    public $header_type = GESHI_HEADER_PRE;			//	The type of header to use.
    ...
    public $time = 0;								//	The time it took to parse the code
    public $header_content = '';					//	The content of the header block
    public $footer_content = '';					//	The content of the footer block
    public $header_content_style = '';				//	The style of the header block
    public $footer_content_style = '';				//	The style of the footer block
    public $force_code_block = false;				//	Tells if a block around the highlighted source should be forced if not using line numbering
    public $link_styles = array();					//	The styles for hyperlinks in the code
    public $enable_important_blocks = false;		//	Whether important blocks should be recognised or not @deprecated
    public $important_styles = 'font-weight: bold; color: red;'; // Styles for important parts of the code @deprecated
    public $add_ids = false;						//	Whether CSS IDs should be added to the code
    public $highlight_extra_lines = array();		//	Lines that should be highlighted extra
    public $highlight_extra_lines_styles = array();	//	Styles of lines that should be highlighted extra
    public $highlight_extra_lines_style = 'background-color: #ffc;';	//	Styles of extra-highlighted lines
    public $line_ending = null;						//	If null, nl2br() will be used on the result string. Otherwise, all instances of \n will be replaced with $line_ending
    public $line_numbers_start = 1;					//	Number at which line numbers should start at
    public $overall_style = 'font-family:monospace;';	//	The overall style for this code block
    public $code_style = 'font: normal normal 1em/1.2em monospace; margin:0; padding:0; background:none; vertical-align:top;';	//	The style for the actual code
    public $overall_class = '';						//	The overall class for this code block
    public $overall_id = '';						//	The overall ID for this code block
    public $line_style1 = 'font-weight: normal; vertical-align:top;';	//	Line number styles
    public $line_style2 = 'font-weight: bold; vertical-align:top;';	//	Line number styles for fancy lines
    public $table_linenumber_style = 'width:1px;text-align:right;margin:0;padding:0 2px;vertical-align:top;';	//	Style for line numbers when GESHI_HEADER_PRE_TABLE is chosen
    public $line_numbers = GESHI_NO_LINE_NUMBERS;	//	Flag for how line numbers are displayed
    public $allow_multiline_span = true;			//	Flag to decide if multi line spans are allowed. Set it to false to make sure each tag is closed before and reopened after each linefeed.
    public $line_nth_row = 0;						//	The "nth" value for fancy line highlighting
    public $tab_width = 4;							//	The size of tab stops
    public $use_language_tab_width = false;			//	Should we use language-defined tab stop widths?
    public $link_target = '';						//	Default target for keyword links
    public $encoding = 'utf-8';						//	The encoding to use for entity encoding NOTE: Used with Escape Char Sequences to fix UTF-8 handling (cf. SF#2037598)
    public $keyword_links = true;					//	Should keywords be linked?
    public $loaded_language = '';					//	Currently loaded language file
    public $parse_cache_built = false;				//	Wether the caches needed for parsing are built or not
    public $_kw_replace_group = 0;					//	Workaround for Suhosin Patch with disabled /e modifier
    public $_rx_key = 0;
    private $_hmr_before = '';						//	Some "callback parameters" for handle_multiline_regexps
     ...
     *	Sets the language for this object. Since 1.0.8 won't reset language-settings by default anymore! if you need this set $force_reset = true
     *
	 *	@access public
     *	@param string
     *	@since 1.0.0
     ...
     *	This path is relative to the directory of the script that included geshi.php, NOT geshi.php itself.
     *
	 *	@access public
     *	@param string
    ...
    public function set_language_path($path) {
        //	Security fixes to prevent external directories using fopen wrappers.
        if(strpos($path,':')) {
    ...
    /**
	 *	function get_language_fullname
    ...
    /**
	 *	function set_code_style
     *	Sets style for the actual code. Should be a string containing valid css. If $preserve_defaults is true, styles are merged with default styles, 
	 *	with user defined styles having priority. Use this method to override any style changes you made to the line numbers if you are using line 
	 *	numbers, else the line of code will have the same style as the line number! Consult the GeSHi documentation for more information about this.
     *
	 *	@access public
     *	@param string style
     *	@param boolean preserve_defaults
    ...
    /**
	 *	function enable_multiline_span
    ...
    /**
	 *	function set_keyword_group_style
    ...
    /**
	 *	function set_keyword_group_highlighting
    ...
    /**
	 *	function set_comments_highlighting
    ...
    /**
	 *	function set_escape_characters_style
    ...
    /**
	 *	function set_escape_characters_highlighting
    ...
    /**
	 *	function set_brackets_style
     * Sets the styles for brackets. If $preserve_defaults is
     * true, then styles are merged with the default styles, with the
     * user defined styles having priority @deprecated In favour of set_symbols_style
     *
    ...
    /**
	 *	function set_brackets_highlighting
     * Turns highlighting on/off for brackets @deprecated In favour of set_symbols_highlighting
     *
    ...
    /**
	 *	function set_symbols_highlighting
    ...
    /**
	 *	function set_strings_highlighting
    ...
    /**
	 *	function set_numbers_highlighting
    ...
    /**
	 *	function set_methods_highlighting
    ...
    /**
	 *	function set_regexps_highlighting
    ...
    /**
	 *	function set_use_language_tab_width
     ...
     * Disables all highlighting @deprecated
     *
     * @since 1.0.0
     */
    ...
    /**
	 *	function set_header_content_style
    ...
    /**
	 *	function set_footer_content_style
    ...
    /**
	 *	function set_url_for_keyword_group
    ...
    /**
	 *	function enable_important_blocks
     *	Sets whether context-important blocks are highlighted @deprecated
     *
    ...
    /**
	 *	function highlight_lines_extra
     *
     * @param mixed lines
     * @param string style
     * @since 1.0.7
    ...
    /**
     *	function set_highlight_lines_extra_style
     *
    ...
    /**
	 *	function start_line_numbers_at
            ...
            if ($highlight_part) {
                ...
                for ($i = 0; $i < $length; ++$i) {
                    ...
                    if ($string_started && ($i != $next_comment_regexp_pos)) {
                        ...
                        $string = "<span$string_attributes>" . Views_Geshi_Main::hsc($char);
                        $start = $i + $char_len;
                        $string_open = true;
                        ...
                        do {
                            ...
                            if($simple_escape < $next_escape_regexp_pos &&
                                ...
                                $string .= "<span$escape_char_attributes>" .
                                    Views_Geshi_Main::hsc($this->language_data['ESCAPE_CHAR']);
...
//        $result = substr($result, 1, -1);
    ...
    /**
	 *	function handle_keyword_replace
    ...
    /**
	 *	function handle_regexps_callback
    ...
    /**
	 *	function handle_multiline_regexps
...
if (!function_exists('geshi_highlight')) {
    ...
    function geshi_highlight($string, $language, $path = null, $return = false) {
        $geshi = new Views_Geshi_Main($string, $language, $path);
