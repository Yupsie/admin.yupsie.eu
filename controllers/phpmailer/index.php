...
/**
 *	class Main
 *	PHP email transport class. (Note: Requires PHP version 5 or later)
 *
 *	@package PHPMailer
 *	@author Andy Prevost
 ...
 */
class C_Phpmailer_Main {

	public $Priority = 3;				// 	Email priority (1 = High, 3 = Normal, 5 = low)
	public $CharSet = 'utf-8';			// 	Sets the CharSet of the message
	public $ContentType = 'text/plain'; // 	Sets the Content-type of the message
	public $Encoding = '8bit';			// 	Sets the Encoding of the message. Options for this are "8bit", "7bit", "binary", "base64", and "quoted-printable"
	public $ErrorInfo = '';				// 	Holds the most recent mailer error message
	public $From = 'noreply@yupsie.nl';	// 	Sets the From email address for the message
	public $FromName = 'Yupsie No-Reply';// Sets the From name of the message
	public $Sender = '';				// 	Sets the Sender email (Return-Path) of the message.  If not empty, will be sent via -f to sendmail or as 'MAIL FROM' in smtp mode
	public $Subject = '';				// 	Sets the Subject of the message
	public $Body = '';					// 	Sets the Body of the message.  This can be either an HTML or text body. If HTML then run IsHTML(true)
	public $AltBody = '';				// 	Sets the text-only body of the message. This automatically sets the email to multipart/alternative. This body can be read by mail clients that do not have HTML email capability such as mutt. Clients that can read HTML will view the normal Body
	...
	public $PluginDir = '/controllers/phpmailer/';	// 	Path to PHPMailer plugins.  Useful if the SMTP class is in a different directory than the PHP include path

	public $ConfirmReadingTo = '';		// 	Sets the email address that a reading confirmation will be sent
	public $Hostname = '';				// 	Sets the hostname to use in Message-Id and Received headers and as default HELO string. If empty, the value returned by SERVER_NAME is used or 'localhost.localdomain'
	public $MessageID = '';				// 	Sets the message ID to be used in the Message-Id header. If empty, a unique id will be generated
	public $Host = 'localhost';			// 	Sets the SMTP hosts.  All hosts must be separated by a semicolon.  You can also specify a different port for each host by using this format: [hostname:port] (e.g. "smtp1.example.com:25;smtp2.example.com"). Hosts will be tried in order
	public $Port = 25;					// 	Sets the default SMTP server port

	public $Helo = '';					// 	Sets the SMTP HELO of the message (Default is $Hostname)
	public $SMTPSecure = '';			// 	Sets connection prefix. Options are "", "ssl" or "tls"
	public $SMTPAuth = false;			// 	Sets SMTP authentication. Utilizes the Username and Password variables
	public $Username = '';				// 	Sets SMTP username
	public $Password = '';				// 	Sets SMTP password

	public $Timeout = 10;				// 	Sets the SMTP server timeout in seconds. This function will not work with the win32 version

	public $SMTPDebug = false;			// 	Sets SMTP class debugging on or off

	public $SMTPKeepAlive = false;		// 	Prevents the SMTP connection from being closed after each mail sending. If this is set to true then to close the connection requires an explicit call to SmtpClose()

	public $SingleTo = false;			// 	Provides the ability to have the TO field process individual emails, instead of sending to entire TO addresses

	public $SingleToArray = array(); 	// 	If SingleTo is true, this provides the array to hold the email addresses

	public $LE = "\n";					// 	Provides the ability to change the line ending

	public $DKIM_selector = 'phpmailer';// 	Used with DKIM DNS Resource Record optional, in format of email address 'you@yourdomain.com'
	...
	public $Version = '5.1';			// Sets the PHPMailer Version number
	 ...
	 *	Check that a string looks roughly like an email address should. Tries to use PHP built-in validator in the filter extension (from PHP 5.2), falls back to a reasonably competent regex validator. Conforms approximately to RFC2822
	 *
	 *	@access public
	...
	public function SmtpConnect() {
		if (is_null($this->smtp)) {
		  	$this->smtp = new C_Phpmailer_Smtp();
		}
	 ...
	 *	@access public
	 *	@param string name
	 *	@param mixed value Value (NOTE: will not work with arrays, there are no arrays to set/reset)
	 *	@return boolean
	 */
